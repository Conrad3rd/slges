...
foreach ($r_hashs as $r_hash) {
	...
	if ($result->num_rows > 0) {
		...
		while($row = $result->fetch_assoc()) {
			$hash_id = $row['hash_id'];
			$hash_name = $row['hash'];
			$r_line = "<span class='hashColl'><a class='text-deco' href='01_search_gallery.php?hash_id=$hash_id'>$hash_name<a class='text-deco hashPlus' href='01_insert.php?hash_id=$hash_id&bild_id=$bild_id'>+</a></a></span>";
			echo "
			$r_line
			";
		}
